...
var date1 = new Date(); // Get current date and time for clock 1

let hr1 = date1.getHours(); // Get current hour for clock 1
let min1 = date1.getMinutes(); // Get current minute for clock 1
let sec1 = date1.getSeconds(); // Get current second for clock 1

let hrPosition1 = (hr1 * 360 / 12) + (min1 * (360 / 60) / 12); // Hour hand degrees for clock 1
let minPosition1 = (min1 * 360 / 60) + (sec1 * (360 / 60) / 60); // Minute hand degrees for clock 1
let secPosition1 = sec1 * 360 / 60; // Second hand degrees for clock 1

function runClock1() {

    hrPosition1 = hrPosition1 + ((360 / 12) / 3600);
    minPosition1 = minPosition1 + (1 / 60) * (360 / 60);
    secPosition1 = secPosition1 + (360 / 60); // Move the second hand one second forward
...
var interval = setInterval(runClock1, 1000); // Run the clock 1 function every second
var interval = setInterval(runClock2, 1000); // Run the clock 2 function every second