...
// Sessions are used for lightweight abuse controls (form token + timing).
...
// CSRF-ish token to reduce automated submissions (not meant as strong auth)
if (empty($_SESSION['contact_form_token'])) {
	try {
		$_SESSION['contact_form_token'] = bin2hex(random_bytes(32));
	} catch (Exception $e) {
		$_SESSION['contact_form_token'] = bin2hex(openssl_random_pseudo_bytes(32));
	}
}
...
if (isset($_POST['submit'])) { // data posted
	...
	if (empty($formToken) || empty($sessionToken) || !hash_equals($sessionToken, $formToken)) {
		$errorText .= "\t\t\t\t\t<p>Please refresh the page and try again.</p>\r";
		$output_form = true;
	} else {
		$renderedAt = (int) ($_SESSION['contact_form_rendered_at'] ?? 0);
		if ($renderedAt > 0 && (time() - $renderedAt) < 2) {
			$errorText .= "\t\t\t\t\t<p>Please wait a moment and try again.</p>\r";
			$output_form = true;
		} else {
			$ip = (string) ($_SERVER['REMOTE_ADDR'] ?? 'unknown');
			$userAgent = (string) ($_SERVER['HTTP_USER_AGENT'] ?? 'unknown');
			$rateLimitKey = $ip . '|' . substr($userAgent, 0, 120);
			$rl = rate_limit_check_and_record($rateLimitKey, 5, 3600, 8);
			if (!($rl['allowed'] ?? true)) {
				$retryAfter = (int) ($rl['retryAfter'] ?? 60);
				$errorText .= "\t\t\t\t\t<p>Too many messages sent. Please try again in " . $retryAfter . " seconds.</p>\r";
				$output_form = true;
			}
		}
	}

	if ($output_form === true && !empty($errorText)) {
		// Skip processing if abuse controls failed.
	} else {
		...
		// Honeypot (spam trap): if filled, silently pretend success and skip sending mail.
		if (!empty($specialInstructions)) {
			// Rotate token to prevent replay loops.
			try {
				$_SESSION['contact_form_token'] = bin2hex(random_bytes(32));
			} catch (Exception $e) {
				$_SESSION['contact_form_token'] = bin2hex(openssl_random_pseudo_bytes(32));
			...
			if (
				...
				if ($validEmailAddress !== false && filter_var($validEmailAddress, FILTER_VALIDATE_EMAIL) === false) {
...
// Update render time whenever we show the form.
if ($output_form) {
...
<head>
	...
	<meta name="description"
		content="Frank Jamison — math educator, STEM enthusiast, and student advocate. Portfolio and contact information.">
	<link rel="canonical" href="https://frankjamison.com/">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<meta name="robots" content="all,follow">

	<!-- Open Graph / social sharing -->
	<meta property="og:title" content="Frank Jamison | Portfolio">
	<meta property="og:description"
		content="Math educator • STEM enthusiast • student advocate. View my work and get in touch.">
	<meta property="og:type" content="website">
	<meta property="og:url" content="https://frankjamison.com/">
	<meta property="og:image" content="https://frankjamison.com/img/frankcolor.jpg">
	...
	<script type="application/ld+json">
		{
			...
			"@graph": [
				{
					"@type": "WebSite",
					"name": "Frank Jamison | Portfolio",
					"url": "https://frankjamison.com/"
				},
				{
					"@type": "Person",
					"name": "Frank Jamison",
					...
					"image": "https://frankjamison.com/img/frankcolor.jpg",
					"jobTitle": "Math Educator",
					"description": "Math educator, STEM enthusiast, and student advocate."
				}
			]
		}
	</script>
			...
			<div class="container clearfix">
				<div class="row">
					<div class="col-md-8 col-md-offset-2 col-sm-12">
						...
						<h1>I am Frank Jamison...</h1>
						<h2 class="h3">Math Educator • STEM Enthusiast • Student Advocate</h2>
				...
				<div class="container">
					<div class="navbar-header">
						<button type="button" title="Website Navigation" data-toggle="collapse"
							data-target="#navigation" class="navbar-btn btn-sm navbar-toggle"
							aria-label="Toggle navigation" aria-controls="navigation" aria-expanded="false">
				...
				<div class="row">
					<div class="col-md-6">
						...
						<p>My academic journey has been expansive and intentional. It began at Mt. San Jacinto College,
							...
							Social/Behavioral Science. I went on to complete a Bachelor of Science in Computer Science
							at
						...
						<p>Currently, I'm pursuing a Master of Education in Inspired Teaching and Learning at National
							University, along with a Preliminary Single Subject Teaching Credential in Mathematics.
							These
							...
							students.</p>
						<p>I'm also working as a long-term substitute teacher for San Jacinto Unified School District,
							where
							I provide consistent academic instruction and classroom support while completing my
							credential
							program. In addition, I serve as a substitute teacher for Hemet Unified, stepping into a
							variety
							of roles to support schools across the district.</p>
						<p>Previously, I worked as an Instructional Laboratory Technician I in the Physics Department at
							Mt.
							San Jacinto College, where I supported STEM instruction, managed lab operations, and served
							on
						...
						<p>Outside of education, I'm a passionate philatelist, specializing in U.S. stamp grading and
							...
							thrill of cataloging rare and classic issues. When I'm not grading stamps or lesson
							planning,
							you’ll likely find me deep in a Dungeons & Dragons campaign—because sometimes the best way
							to
							understand probability is to roll a d20.</p>
						<p>Thanks for stopping by. Feel free to explore my portfolio to see how my experience in
							education
						...
						<div class="row">
							<div class="col-sm-4">
								<div class="box">
									...
									<p>I develop and deliver math lessons that build conceptual understanding,
										real-world
							...
							<div class="col-sm-4">
								<div class="box">
									...
									<p>Through one-on-one tutoring and classroom teaching, I support students with mild
										to
										moderate disabilities, including ADHD and autism. I use clear routines,
										scaffolded
							...
							<div class="col-sm-4">
								<div class="box">
									...
									<p>Whether stepping into a new classroom as a substitute or leading instruction
										long-term, I create learning environments grounded in mutual respect,
										consistency,
										and high expectations. I prioritize connection and communication as tools for
										both
				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="row">
							<div class="col-sm-4">
								<div class="box">
									<a href="https://jamisonstamps.com/" title="JamisonStamps.com" target="_blank"
										rel="noopener noreferrer"
										aria-label="Visit JamisonStamps.com (opens in a new tab)"><img
											src="img/Portfolio-JamisonStamps.com.png"
											alt="Screenshot of JamisonStamps.com" class="img-responsive"></a>
									<div class="projectTitle">
										<h5 class="project">JamisonStamps.com<br>Business Website for<br>my stamp
											dealing
											business</h5>
										<p class="text-center" style="margin-top: 10px;">
											<a href="https://jamisonstamps.com/" target="_blank"
												rel="noopener noreferrer"
												aria-label="Visit JamisonStamps.com (opens in a new tab)">
												Visit JamisonStamps.com
											</a>
										</p>
									</div>
								</div>
							...
							<div class="col-sm-4">
								<div class="box">
									<a href="https://passionateteachingjourney.com/"
										title="PassionateTeachingJourney.com" target="_blank" rel="noopener noreferrer"
										aria-label="Visit PassionateTeachingJourney.com (opens in a new tab)"><img
											src="img/Portfolio-PassionateTeachingJourney.com.png"
											alt="Screenshot of PassionateTeachingJourney.com"
											class="img-responsive"></a>
									<div class="projectTitle">
										<h5 class="project">PassionateTeachingJourney.com<br>Personal Blog
											Showcasing<br>Educational Accomplishments</h5>
										<p class="text-center" style="margin-top: 10px;">
											<a href="https://passionateteachingjourney.com/" target="_blank"
												rel="noopener noreferrer"
												aria-label="Visit PassionateTeachingJourney.com (opens in a new tab)">
												Visit PassionateTeachingJourney.com
											</a>
										</p>
									</div>
								</div>
							...
							<div class="col-sm-4">
								<div class="box">
									<a href="http://DnD5eTools.com" title="DnD 5e Tools Website" target="_blank"
										rel="noopener noreferrer"
										aria-label="Visit DnD5eTools.com (opens in a new tab)"><img
											src="img/Portfolio-DnD5eTools.com.png" alt="Screenshot of DnD5eTools.com"
											class="img-responsive"></a>
									<div class="projectTitle">
										<h5 class="project">DnD5eTools.com<br>Online Tools For<br>Dungeons &amp; Dragons
											5th
											Edition</h5>
										<p class="text-center" style="margin-top: 10px;">
											<a href="http://DnD5eTools.com" target="_blank" rel="noopener noreferrer"
												aria-label="Visit DnD5eTools.com (opens in a new tab)">
												Visit DnD5eTools.com
											</a>
										</p>
					...
					<div class="col-md-12">
						...
						<div class="row">
							<div class="col-sm-6">
								...
								<p>The journey of education continues to unfold like a map drawn in real time-each
									landmark a lesson, each path a purpose. My mission remains rooted in growth, fueled
									by
									curiosity, and driven by a commitment to the success of every student I encounter.
									These
				...
				<div class="row">
					...
					<div class="col-md-12">
						...
						if ($output_form) {
								...
								<div class="col-md-6">
									...
									<form action="<?= $_SERVER['PHP_SELF'] . '#contact' ?>" method="post"
										...
										<input type="hidden" name="form_token"
											value="<?= htmlspecialchars($_SESSION['contact_form_token'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
										...
										<div class="controls">
											...
											<div class="form-group">
												...
												<input type="email" name="emailAddress" id="emailAddress"
													value="<?= htmlspecialchars($emailAddress, ENT_QUOTES, 'UTF-8') ?>"
											...
											<div class="form-group">
												...
												<textarea rows="4" name="comment" id="comment"
													...
													class="form-control"><?= htmlspecialchars($comment, ENT_QUOTES, 'UTF-8') ?></textarea>
											</div>
											...
											<div class="form-group special-instructions">
												...
												<input type="text" name="specialInstructions" id="specialInstructions"
													...
													placeholder="Leave this field blank" class="form-control">
											</div>
										</div>
									</form>
								</div>
								<div class="col-md-6">
									<p>If you like my portfolio and wish to contact me about an employment opportunity,
										please
							...
							<?php
							$redirectTarget = $_SERVER['PHP_SELF'] . '#contact';
